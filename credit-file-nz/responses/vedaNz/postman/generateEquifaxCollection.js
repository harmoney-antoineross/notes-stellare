...
// Read and parse CSV 
try {
  const csvData = fs.readFileSync('Equifax Consumer-Table.csv', 'utf-8');
  ...
  const testItems = consumerRecords.map(record => {
    const xmlBody = generateXmlBody(record);
    
    return {
      name: `${getTestCaseType(record)} - ${record['First Name']} ${record['Surname']}`,
            ...
            <request mode="test" version="V2.0">
                ...
                <individual>
                    <individual-name>
                        ...
                        ${record['Middle Name'] ? `<middle-names>${record['Middle Name']}</middle-names>` : ''}
                    </individual-name>
                    <gender type="${record['Gender']?.toLowerCase() === 'm' ? 'male' : 'female'}"/>
